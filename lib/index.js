...
export function zone(node, startName, endName, handler) {
  ...
  visit(node, (node, index, parent) => {
    ...
    if (startName === endName) {
      ...
      const startRegex = new RegExp(startName);
      const endRegex = new RegExp(endName);
      if (info && info.node.value.match(startRegex)) {
        type = 'start'
      } else if (info && info.node.value.match(endRegex)) {
        type = 'end'
      } else {
        type = undefined
      }
